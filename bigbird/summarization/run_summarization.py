...
def model_fn_builder(transformer_config):
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (llh, logits, pred_ids), _ = model.get_logit() ## 1) spars softmax 결과 값, 2) softmax를 수행하기전 값, 3) beam search를 수행한 과결 값

    padded_cross_entropy = PaddedCrossEntropyLoss(
        logits, labels,
        transformer_config["label_smoothing"],
        transformer_config["vocab_size"])
    total_loss = padded_cross_entropy.get_loss()

    tvars = tf.compat.v1.trainable_variables()
    utils.LogVariable(tvars, transformer_config["ckpt_var_list"])

    output_spec = None
    if mode == tf.estimator.ModeKeys.TRAIN:

      # learning rate 계산
      opt_model = optimization.LinearWarmupRSQRTDecayLR(
          init_lr=transformer_config["learning_rate"],
          hidden_size=transformer_config["hidden_size"],
          num_warmup_steps=transformer_config["num_warmup_steps"])
      learning_rate = opt_model.get_learning_rate()

      optimizer = optimization.Optimizer(transformer_config, learning_rate)
      ...
      logging_hook = [tf.compat.v1.train.LoggingTensorHook({"loss is -> ": total_loss}, every_n_iter=10)]
      ...
      logging_hook = [tf.compat.v1.train.LoggingTensorHook({"predic ids -> ": pred_ids}, every_n_iter=2)]
      output_spec = tf.compat.v1.estimator.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
...
class PaddedCrossEntropyLoss():
    def __init__(self, logits, labels, smoothing, vocab_size):
      ...
      with tf.name_scope("loss"):
        ...
        if labels is not None:
          ...
          with tf.name_scope("smoothing_cross_entropy"): # smoothing 기법 적
            confidence = 1.0 - smoothing # high confidence 생성
            vocab_float = tf.cast(vocab_size - 1, tf.float32)
            low_confidence = (1.0 - confidence) / vocab_float # low confidence 생성 0.1
            soft_targets = tf.one_hot(
                labels,
                depth=vocab_size,
                on_value=confidence, # on_value: represented 된 위치는 0.9,
                off_value=low_confidence) # off_value: represented 되지 않는 위치는 0에 가까운 아주 작은값 사용
            xentropy = tf.nn.softmax_cross_entropy_with_logits(
                logits=logits, labels=soft_targets) # 정답과 모델이 만들어낸 정답 2개의 cross entropy 계산

            # Calculate the best (lowest) possible value of cross entropy, and
            # subtract from the cross entropy loss.
            normalizing_constant = -( # normalize constant  생성
                confidence * tf.math.log(confidence) + vocab_float *
                low_confidence * tf.math.log(low_confidence + 1e-20))
            xentropy -= normalizing_constant # normalizing constant 빼기

          weights = tf.cast(tf.not_equal(labels, 0), tf.float32)
          self.loss = tf.reduce_sum(xentropy) / tf.reduce_sum(weights) # 라벨에 답이 있는 부분만 계산하기 위해서 weight sum
