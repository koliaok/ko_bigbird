...
class LinearWarmupLinearDecay(object):

  def __init__(self, init_lr, num_train_steps, num_warmup_steps):

    """Calculate learning rate with linear warmup and linear decay."""
    global_step = tf.compat.v1.train.get_or_create_global_step() # global step 가져옴

    learning_rate = tf.constant(value=init_lr, shape=[], dtype=tf.float32)# laerning rate tensor로 변환

    # Implements linear decay of the learning rate.
    learning_rate = tf.compat.v1.train.polynomial_decay( # learning rate를 조금씩 작게 만들어 주는 계산
        learning_rate,
        global_step,
        num_train_steps,
    ...
    if num_warmup_steps: # warmup step적용하
      global_steps_int = tf.cast(global_step, tf.int32) #global step int32로 만들기
      warmup_steps_int = tf.constant(num_warmup_steps, dtype=tf.int32) # warmup step tensor로 가져옴

      global_steps_float = tf.cast(global_step, tf.float32) # float 형태
      warmup_steps_float = tf.cast(num_warmup_steps, tf.float32) # float 형태

      warmup_percent_done = global_steps_float / warmup_steps_float # 전체 warmup step중 global step의 비율
      warmup_learning_rate = init_lr * warmup_percent_done # warmup percent를 초기 learning rate에 적

      is_warmup = tf.cast(global_steps_int < warmup_steps_int, tf.float32) # global step보다 warmup step이 클 경우 1.0, 아닐 경우 0
      # global step 이 warmup step 보다 작을때 까지는  warmup_laerning rate를 사용하고,
      # gloabl step이 warmup step보다 큰 경우부터 기존 laerning rate사용
      self.learning_rate = (
...
class LinearWarmupRSQRTDecayLR(object):

  def __init__(self, init_lr, hidden_size, num_warmup_steps):
    """Calculate learning rate with linear warmup and rsqrt decay."""
    num_warmup_steps = tf.cast(num_warmup_steps, tf.float32) # warm step을 tf cast : 100
    global_step = tf.compat.v1.train.get_or_create_global_step()
    global_step = tf.cast(global_step, tf.float32)

    learning_rate = tf.constant(value=init_lr, shape=[], dtype=tf.float32) # 초기 learning rate
    learning_rate *= tf.math.rsqrt(tf.cast(hidden_size, tf.float32)) # learning rate에 hidden_size의 squar root의 역수를 구해서 곱
    # Apply linear warmup
    learning_rate *= tf.minimum(1.0, global_step / num_warmup_steps) # warmup step에 가까워 질수록 learning rate값이 커짐
    # Apply rsqrt decay
    learning_rate *= tf.math.rsqrt(tf.maximum(global_step, num_warmup_steps)) # global_step과 warmup step중에 큰 값을 rsqrt로 사
