  ...
  def operation(self,
    ...
    with tf.compat.v1.variable_scope("attention"):
      with tf.compat.v1.variable_scope("self") as sc:
        normalized_layer_input = self.first_layer_norm.operation(layer_input)
        self_attention_output = self.self_attn_layer.operation(
      ...
      # with `layer_input`.
      with tf.compat.v1.variable_scope("output"): # projection 계산
        self_attention_output = self.self_proj_layer.operation(self_attention_output) # (2, 256, 1024)
        self_attention_output = utils.dropout(self_attention_output, # drop out
                                              self.hidden_dropout_prob,
                                              training)
        self_attention_output = self_attention_output + layer_input

      with tf.compat.v1.variable_scope("encdec") as sc:
        normalized_self_attention_output = self.second_layer_norm.operation(
            self_attention_output)
        attention_output = self.cross_attn_layer.operation(
      ...
      with tf.compat.v1.variable_scope("encdec_output"):
        attention_output = self.cross_proj_layer.operation(attention_output)
    ...
    with tf.compat.v1.variable_scope("intermediate"):
      normalized_attention_output = self.third_layer_norm.operation(attention_output)
...
class DecoderStack(tf.compat.v1.layers.Layer):
  ...
  def operation(self,
    ...
    attention_mask = tf.expand_dims(tf.expand_dims(encoder_mask, 1), 1) # encoder mask (2, 1, 1 3072)로 확장

    # if self.params["use_gradient_checkpointing"]::
    #   decoder_layer = recompute_gradient(decoder_layer)

    if self.params["norm_type"] == "postnorm": # decoder에서는 prenorm이므로 사용하지 않는
      decoder_inputs = self.layer_norm.operation(decoder_inputs)

    # attention 시작 부분 16 layer
...
def create_self_attention_mask(length):
  with tf.name_scope("decoder_self_attention_mask"):
    valid_locs = tf.linalg.band_part(tf.ones([length, length]), -1, 0) # decoder masking을 위해서 [[1, 0, 0], [1, 1, 0], [1, 1, 1]] 이런 형식
    valid_locs = tf.reshape(valid_locs, [1, 1, length, length])
  return valid_locs # shape (1, 1, 256, 256) 형태 decoder mask 생성
...
def left2right_decode(symbols_to_logits_fn,
  ...
  """
  dtype = tf.int32
  start_symbols = tf.expand_dims(start_symbols, 1) # 시작 dim에 dim 1을 확장
  ...
  if beam_size == 1:
    ...
    length_norm_fn = beam_search.length_normalization(beam_start, beam_alpha, # beam search length 가 길어지는 것에 대한 정규를 위한 function
                                                      beam_min, beam_max, -1e3)

    init_dec_BxT = tf.concat([tf.cast(start_symbols, dtype=tf.int32),# (2, 256)
                              tf.zeros([batch_size, max_decode_len-1],
                                       dtype=tf.int32)], axis=1) #시작 token 외 모두 0으로 한 초기 decoder tensor 생성
    ...
    return beams[:, 0, :] # 가장 큰값만 return [2, 1, 256]
